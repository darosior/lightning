...
def test_gossip_pruning(node_factory, bitcoind):
    ...
    wait_for(
        lambda: [c["active"] for c in l1.rpc.listchannels()["channels"]] == [True] * 4
    )
    wait_for(
        lambda: [c["active"] for c in l2.rpc.listchannels()["channels"]] == [True] * 4
    )
    wait_for(
        lambda: [c["active"] for c in l3.rpc.listchannels()["channels"]] == [True] * 4
    )
    ...
    l1.daemon.wait_for_log(
        "Pruning channel {} from network view".format(scid2), timeout=80
    )
    ...
    assert scid1 not in [
        c["short_channel_id"] for c in l3.rpc.listchannels()["channels"]
    ]
...
def test_gossip_timestamp_filter(node_factory, bitcoind):
    ...
    wait_for(
        lambda: ["alias" in node for node in l4.rpc.listnodes()["nodes"]]
        == [True, True, True]
    )

    msgs = l4.query_gossip(
...
def test_connect_by_gossip(node_factory, bitcoind):
    ...
    l1, l2, l3 = node_factory.get_nodes(
        ...
        opts=[
            ...
            {},
            {"dev-allow-localhost": None, "log-level": "io"},
        ],
    )
    l2.rpc.connect(l3.info["id"], "localhost", l3.port)
    ...
    l2.daemon.wait_for_logs(
        ["Received node_announcement for node {}".format(l3.info["id"])]
    )
...
@unittest.skipIf(
    not DEVELOPER,
    "DEVELOPER=1 needed to speed up gossip propagation, would be too long otherwise",
)
...
def test_gossip_weirdalias(node_factory, bitcoind):
    ...
    opts = [{"alias": weird_name}, {"alias": normal_name}]
    l1, l2 = node_factory.get_nodes(2, opts=opts)
    ...
    assert l2.daemon.is_in_log(
        "Server started with public key .* alias {}".format(normal_name)
    )
    ...
    l1.daemon.wait_for_log(
        "Received node_announcement for node {}".format(l2.info["id"])
    )
    l2.daemon.wait_for_log(
        "Received node_announcement for node {}".format(l1.info["id"])
    )
...
def test_routing_gossip_reconnect(node_factory):
    ...
    l1, l2, l3 = node_factory.get_nodes(
        3,
        opts=[
            {"disconnect": disconnects, "may_reconnect": True},
            {"may_reconnect": True},
            {},
        ],
    )
...
def test_gossip_no_empty_announcements(node_factory, bitcoind):
    ...
    l1, l2, l3, l4 = node_factory.line_graph(
        ...
        opts=[
            {"log-level": "io"},
            {"log-level": "io"},
            {"disconnect": ["+WIRE_CHANNEL_ANNOUNCEMENT"], "may_reconnect": True},
            {"may_reconnect": True},
        ],
        fundchannel=False,
    )
...
def test_routing_gossip(node_factory, bitcoind):
    ...
    def check_gossip(n):
        ...
        logging.debug(
            "Node {id} is missing channels {chans}".format(
                id=n.info["id"], chans=missing
            )
        )
...
def test_gossip_query_channel_range(node_factory, bitcoind, chainparams):
    ...
    scid12 = only_one(l1.rpc.listpeers(l2.info["id"])["peers"])["channels"][0][
        "short_channel_id"
    ]
    scid23 = only_one(l3.rpc.listpeers(l2.info["id"])["peers"])["channels"][0][
        "short_channel_id"
    ]
    ...
    msgs = l2.query_gossip(
        "query_channel_range", chainparams["chain_hash"], 0, 1000000, filters=["0109"]
    )
    encoded = (
        subprocess.run(
            ["devtools/mkencoded", "--scids", "00", scid12, scid23],
            check=True,
            timeout=TIMEOUT,
    ...
    msgs = l2.query_gossip(
        "query_channel_range", genesis_blockhash, 0, block12, filters=["0109"]
    )
    # reply_channel_range == 264
    assert msgs == [
        "0108"
    ...
    # Does include scid12
    msgs = l2.query_gossip(
        "query_channel_range", genesis_blockhash, 0, block12 + 1, filters=["0109"]
    )
    encoded = (
        subprocess.run(
            ["devtools/mkencoded", "--scids", "00", scid12],
            check=True,
            timeout=TIMEOUT,
            stdout=subprocess.PIPE,
        )
    ...
    msgs = l2.query_gossip(
        "query_channel_range", genesis_blockhash, 0, block23, filters=["0109"]
    )
    encoded = (
        subprocess.run(
            ["devtools/mkencoded", "--scids", "00", scid12],
    ...
    msgs = l2.query_gossip(
        "query_channel_range", genesis_blockhash, block23, 1, filters=["0109"]
    )
    encoded = (
        subprocess.run(
            ["devtools/mkencoded", "--scids", "00", scid23],
            check=True,
            timeout=TIMEOUT,
            stdout=subprocess.PIPE,
        )
    ...
    # Past both
    msgs = l2.query_gossip(
        "query_channel_range", genesis_blockhash, block23 + 1, 1000000, filters=["0109"]
    )
    # reply_channel_range == 264
    assert msgs == [
        "0108"
    ...
    msgs = l2.query_gossip(
        "query_channel_range", genesis_blockhash, 0, 1000000, filters=["0109"]
    )
    ...
    msgs = l2.query_gossip(
        "query_channel_range", genesis_blockhash, 1, 429496000, filters=["0109"]
    )
    assert len(msgs) == 2
    ...
    l2.daemon.wait_for_log("Set max_scids_encode_bytes to {}".format(2 ** 32 - 1))

    msgs = l2.query_gossip(
        "query_channel_range", genesis_blockhash, 0, 65535, filters=["0109"]
    )
    encoded = (
        subprocess.run(
            ["devtools/mkencoded", "--scids", "01", scid12, scid23, scid34],
            check=True,
            timeout=TIMEOUT,
            stdout=subprocess.PIPE,
        )
...
def test_report_routing_failure(node_factory, bitcoind):
    ...
    for src, dst in [(l1, l2), (l2, l3), (l3, l4), (l4, l1), (l4, l2)]:
        src.rpc.connect(dst.info["id"], "localhost", dst.port)
        print(
            "src={}, dst={}".format(src.daemon.lightning_dir, dst.daemon.lightning_dir)
        )
...
def test_query_short_channel_id(node_factory, bitcoind, chainparams):
    ...
    encoded = (
        subprocess.run(
            ["devtools/mkencoded", "--scids", "00", "1x1x1", "2x2x2"],
            check=True,
            timeout=TIMEOUT,
            stdout=subprocess.PIPE,
        )
    ...
    msgs = l1.query_gossip(
        "query_short_channel_ids", chain_hash, encoded, filters=["0109"]
    )
...
def test_gossip_store_load(node_factory):
    ...
    with open(
        os.path.join(l1.daemon.lightning_dir, TEST_NETWORK, "gossip_store"), "wb"
    ) as f:
        f.write(
            bytearray.fromhex(
                "09"  # GOSSIP_STORE_VERSION
                "000001b0"  # len
                "fea676e8"  # csum
                "5b8d9b44"  # timestamp
    ...
    wait_for(
        lambda: l1.daemon.is_in_log(
            r"gossip_store: Read 1/1/1/0 cannounce/cupdate/nannounce/cdelete from store \(0 deleted\) in 770 bytes"
        )
    )
...
def test_gossip_store_load_amount_truncated(node_factory):
    ...
    wait_for(
        lambda: l1.daemon.is_in_log(
            r"gossip_store: dangling channel_announcement. Moving to gossip_store.corrupt and truncating"
        )
    )
    wait_for(
        lambda: l1.daemon.is_in_log(
            r"gossip_store: Read 0/0/0/0 cannounce/cupdate/nannounce/cdelete from store \(0 deleted\) in 1 bytes"
        )
    )
    assert os.path.exists(
        os.path.join(l1.daemon.lightning_dir, TEST_NETWORK, "gossip_store.corrupt")
    )
...
def test_node_reannounce(node_factory, bitcoind):
    ...
    l1, l2 = node_factory.line_graph(2, opts={"may_reconnect": True, "log-level": "io"})
    bitcoind.generate_block(5)
    ...
    assert only_one(l2.rpc.listnodes(l1.info["id"])["nodes"])["alias"].startswith(
        "JUNIORBEAM"
    )
    ...
    wait_for(
        lambda: only_one(l2.rpc.listnodes(l1.info["id"])["nodes"])["alias"]
        == "SENIORBEAM"
    )
    ...
    assert len(msgs) == 2
    assert (
        bytes("SENIORBEAM", encoding="utf8").hex() in msgs[0]
...
def test_gossip_notices_close(node_factory, bitcoind):
    ...
    l1, l2, l3 = node_factory.get_nodes(
        3, opts=[{"log-level": "io", "allow_bad_gossip": True}, {}, {}]
    )
    ...
    wait_for(lambda: len(l1.rpc.listchannels()["channels"]) == 2)
    wait_for(
        lambda: ["alias" in n for n in l1.rpc.listnodes()["nodes"]] == [True, True]
    )
    ...
    wait_for(
        lambda: only_one(l2.rpc.listpeers(l3.info["id"])["peers"])["channels"][0][
            "state"
        ]
        == "CLOSINGD_COMPLETE"
    )
...
def test_getroute_exclude(node_factory, bitcoind):
    ...
    l1.daemon.wait_for_logs(
        [
            r"update for channel {}/0 now ACTIVE".format(scid15),
            r"update for channel {}/1 now ACTIVE".format(scid15),
            r"update for channel {}/0 now ACTIVE".format(scid54),
            r"update for channel {}/1 now ACTIVE".format(scid54),
        ]
    )
    ...
    with pytest.raises(RpcError):
        l1.rpc.getroute(
            l4.info["id"], 1, 1, exclude=[l3.info["id"], l5.info["id"], chan_l2l4]
        )

    # This doesn't work
    with pytest.raises(RpcError):
        l1.rpc.getroute(
            l4.info["id"], 1, 1, exclude=[chan_l2l3, l5.info["id"], chan_l2l4]
        )
...
def setup_gossip_store_test(node_factory, bitcoind):
    ...
    wait_for(
        lambda: ["alias" in n for n in l2.rpc.listnodes()["nodes"]] == [True, True]
    )
    ...
    wait_for(
        lambda: sum(
            [c["base_fee_millisatoshi"] for c in l2.rpc.listchannels()["channels"]]
        )
        == 21
    )
    ...
    wait_for(
        lambda: [
            c["base_fee_millisatoshi"] for c in l2.rpc.listchannels(scid23)["channels"]
        ]
        == [20, 20]
    )
    ...
    wait_for(
        lambda: [
            c["base_fee_millisatoshi"] for c in l2.rpc.listchannels(scid12)["channels"]
        ]
        == [20, 20]
    )
...
def test_gossip_store_compact_noappend(node_factory, bitcoind):
    ...
    with open(
        os.path.join(l2.daemon.lightning_dir, TEST_NETWORK, "gossip_store.tmp"), "wb"
    ) as f:
...
def test_gossip_store_load_no_channel_update(node_factory):
    ...
    wait_for(
        lambda: l1.daemon.is_in_log(
            "gossip_store: Unupdated channel_announcement at 1. Moving to gossip_store.corrupt and truncating"
        )
    )
    assert os.path.exists(
        os.path.join(l1.daemon.lightning_dir, TEST_NETWORK, "gossip_store.corrupt")
    ...
    with open(
        os.path.join(l1.daemon.lightning_dir, TEST_NETWORK, "gossip_store"), "rb"
    ) as f:
...
def test_gossip_store_compact_on_load(node_factory, bitcoind):
    ...
    wait_for(
        lambda: l2.daemon.is_in_log(
            r"gossip_store_compact_offline: [5-8] deleted, 9 copied"
        )
    )

    wait_for(
        lambda: l2.daemon.is_in_log(
            r"gossip_store: Read 2/4/2/0 cannounce/cupdate/nannounce/cdelete from store \(0 deleted\) in [0-9]* bytes"
        )
    )
...
def test_gossip_no_backtalk(node_factory):
    ...
    l3.daemon.wait_for_logs(
        [r"\[IN\] 0100", r"\[IN\] 0102", r"\[IN\] 0102", r"\[IN\] 0101", r"\[IN\] 0101"]
    )
...
def test_gossip_ratelimit(node_factory, bitcoind):
    ...
    """
    (l3,) = node_factory.get_nodes(1, opts=[{"dev-gossip-time": 1568096251}])
...
@unittest.skipIf(
    not DEVELOPER, "needs a running Tor service instance at port 9151 or 9051"
)
def test_statictor_onions(node_factory):
    ...
    l2 = node_factory.get_node(
        ...
        options={
            ...
            "addr": [
                "statictor:{}/torblob=11234567890123456789012345678901".format(torips)
            ],
        },
    )
    ...
    assert l2.daemon.is_in_log(
        "x2y4zvh4fn5q3eouuh7nxnc7zeawrqoutljrup2xjtiyxgx3emgkemad.onion:9735,127.0.0.1:{}".format(
            l2.port
        )
    )
...
def test_torport_onions(node_factory):
    ...
    l1 = node_factory.get_node(
        ...
        options={
            "bind-addr": "127.0.0.1:{}".format(portA),
            "addr": ["statictor:{}/torport=45321".format(torips)],
        },
    )
    l2 = node_factory.get_node(
        may_fail=True,
        options={
            "bind-addr": "127.0.0.1:{}".format(portB),
            "addr": [
                "statictor:{}/torport=45321/torblob=11234567890123456789012345678901".format(
                    torips
                )
            ],
        },
    ...
    assert l2.daemon.is_in_log(
        "x2y4zvh4fn5q3eouuh7nxnc7zeawrqoutljrup2xjtiyxgx3emgkemad.onion:45321,127.0.0.1:{}".format(
            l2.port
        )
    )
...
def test_gossip_store_upgrade_v7_v8(node_factory):
    ...
    with open(
        ...
        f.write(
            bytearray.fromhex(
                "07000000428ce4d2d8000000000daf00"
...
def test_routetool(node_factory):
    ...
    subprocess.run(
        [
            "devtools/route",
            os.path.join(l1.daemon.lightning_dir, TEST_NETWORK, "gossip_store"),
            l1.info["id"],
            l2.info["id"],
        ],
        check=True,
...
def test_addgossip(node_factory):
    l1, l2 = node_factory.line_graph(
        2, fundchannel=True, wait_for_announce=True, opts={"log-level": "io"}
    )
